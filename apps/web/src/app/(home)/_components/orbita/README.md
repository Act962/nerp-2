...
Esta pasta é o site institucional da ÓRBITA HUB, renderizado na home (`/`).
...
```
data/catalog.ts        as 19 ferramentas da suíte e suas funcionalidades
...
**Soluções** lista as 19 ferramentas em quatro colunas, uma por categoria, com
um ícone cada (`ui/icons.tsx`, desenhos em SVG, sem fonte de ícones).
...
Texturas e imagens da marca vivem em `public/orbita/`. `lib/assets.ts` resolve
...
`/site` é onde o conteúdo do site é editado. O que ele controla:
...
em `(home)/page.tsx`. O fallback sem WebGL lê o mesmo contexto — quem entra sem
...
- `STATS`, em `data/site.ts`, são números de placeholder — agora editáveis em
  `/site`, mas ainda inventados até alguém trocar.
