...
/**
 * Painel do Winthor, como estava antes do catálogo de Integrações — só perdeu o
 * Card e o cabeçalho, que agora são do dialog que o abre.
 */
export function WinthorPanel() {
      ...
      {configured && (
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 rounded-md bg-muted/40 px-3 py-2 text-sm">
          <span className="text-muted-foreground">
            {lastSync ? `Sincronizado ${lastSync}` : "Ainda não sincronizado"}
          </span>
          {status?.configured && (
            <span className="text-muted-foreground">
              · {status.activeSellers} vendedores · {status.factRows} registros
            </span>
          )}
