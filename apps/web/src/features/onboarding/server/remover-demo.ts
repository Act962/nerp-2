...
export async function removerDadosDeExemplo(
  ...
  return prisma.$transaction(async (tx) => {
    ...
    // O pacote por solução: vendas de exemplo primeiro (SaleItem trava o
    // produto), depois CRM, calendário, cupom e metas.
    await tx.sale.deleteMany({ where: { organizationId, isDemo: true } });
    await tx.crmLead.deleteMany({ where: { organizationId, isDemo: true } });
    await tx.crmTag.deleteMany({ where: { organizationId, isDemo: true } });
    await tx.crmFunnel.deleteMany({ where: { organizationId, isDemo: true } });
    ...
    await tx.salesGoalPeriod.deleteMany({
      where: { organizationId, isDemo: true },
    });
