...
- [x] **A resposta entra na conversa como qualquer outra.** O atalho pula o MODELO, não a memória: a resposta vira mensagem de assistente de verdade, o painel a guarda, e ela volta no histórico da mensagem seguinte — então o Astro enxerga o que respondeu ali e não recomeça do zero. Sai com `messageId` próprio, porque o painel desenha a lista por id e duas respostas de atalho colidiriam.
...
- **O atalho pula o modelo, não a conversa.** Chegou-se a supor que a resposta ficaria fora do histórico; não fica. Ela é uma mensagem de assistente como as outras, e a pergunta seguinte a enxerga. O que o reconhecedor recusa — pergunta de sequência — é por ambiguidade da frase, e não por falta de memória.
...
- unit: `features/astro/server/atalhos/fluxo.test.ts` — a resposta do atalho vira mensagem de assistente com papel, texto e id; duas respostas não colidem; acento e quebra de linha sobrevivem. É o teste que garante que o atalho não apaga a conversa.
