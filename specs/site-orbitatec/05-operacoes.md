...
> **18 ferramentas de operação + o ERP integrado** (o NERP aparece aqui como ponte;
> o produto completo tem página própria em `/solucoes/erp/nerp`).
...
| **ORBITA NERP** *(o ERP dentro da suíte)* | O ERP integrado à plataforma: produtos, categorias, estoque, vendas, clientes e dashboard. É a ponte entre a operação comercial e a operação da loja — e sincroniza com o NERP completo. Página própria → `/solucoes/erp/nerp`. |

## Seção "E o ERP?" — a ponte com a operação da loja

> Seção obrigatória. Sem ela a página vende a suíte como se ela vivesse solta do
> negócio — e o maior diferencial da ORBITA é justamente os dois lados serem a
> mesma casa. É também o principal gancho de cross-sell da página.

- **Título:** A suíte não substitui seu ERP. Ela conversa com ele.
- **Texto:** As ferramentas de operação cuidam do processo entre setores — captar,
  atender, fechar, cobrar, entregar. O **NERP** cuida da operação da loja — produto,
  estoque, venda, caixa, fiscal e financeiro. Os dois compartilham a mesma base:
  o produto cadastrado no ERP aparece na proposta do Forge, a venda fechada vira
  lançamento no Payment, e a meta batida no caixa sobe para o Ranking na mesma hora.
  Sem exportar planilha, sem digitar de novo.
- **Como funciona (3 blocos):**
  1. **Uma base, dois lados.** Produtos, clientes e vendas nascem no ERP e ficam
     disponíveis para todas as ferramentas de operação.
  2. **Sincronização automática e segura.** A troca entre a suíte e o NERP é contínua
     e assinada — nada depende de alguém lembrar de exportar.
  3. **Já tem ERP?** O Orbita Connect integra com WinThor, TOTVS, Oracle, SAP e APIs
     abertas. Você não precisa trocar de ERP para começar a usar a suíte.
- **CTA da seção:** Conhecer o NERP → `/solucoes/erp/nerp` · Ver integrações →
  `/solucoes/ecommerce` *(ou página de integrações, se criada)*
- **Nota interna:** o sync NASA ↔ NERP existe e é assinado por HMAC
  (`SYNC_SHARED_SECRET`/`NERP_BASE_URL`). O app `/nerp` dentro do `nasaex-wey`
  cobre produtos, categorias, estoques, vendas, clientes, dashboard, configurações
  de catálogo/loja e o Ranking de Vendas. **Não** descrever como "ERP completo" nessa
  página — o ERP completo é o NERP, que tem página própria. Aqui é a ponte.
